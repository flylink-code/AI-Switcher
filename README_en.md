...
- **Claude Code subagents**: Explore / Haiku use the catalog subagent slot (prioritizes `gemini-3.7-flash-low`) instead of the current `/model` default; `thinking: disabled` does not rewrite an explicit Gemini suffix or sticky `-low` onto the parent session
- **Model catalog & scheduling**: Gemini **3.6 and 3.7 coexist** (Cloud Code ids are `-high` / `-medium` / `-low`); a same-account 429 walks the level chain (including `3.6-flash-low` → `3.7-flash-*`) with micro-jitter backoffs; recognizes URL-level rate limits and performs seamless upstream endpoint fallback (`daily` → production cluster when encountering `Resource has been exhausted`, resolving burst 429s during Claude Code start/end turns). When quota bars still have remaining, try one more account with a short cooldown (≤15s) instead of parking the whole pool; failed requests log the final attempted model rather than only the initial request ID
...
| sub2api | Antigravity URL-level rate limiting & upstream endpoint fallback | [sub2api](https://github.com/sub2api) |

...
- 版本：工作区为 `1.0.2`（`package.json`、`src-tauri/tauri.conf.json`、`src-tauri/Cargo.toml`）。
...
- 更新后恢复：优雅停代理 + WAL checkpoint + 更长端口重试 + 启动后二次恢复（Codex/代理）
...
- 前端 `tsc --noEmit`：通过（1.0.1 起）
- `proxy` / `app_update` 相关 Rust 单测：通过

## 后续操作建议

1. 推送 `main` 与带注释的 `v1.0.2` 标签，由 GitHub Actions 云端构建发布。
2. Windows 用户优先 NSIS。

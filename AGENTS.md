...
- 版本：工作区已更新为 `0.8.4`（`package.json`、`src-tauri/tauri.conf.json`、`src-tauri/Cargo.toml`）。
- 构建与测试：允许执行本项目的编译和测试；包管理器请使用 Corepack 读取 `packageManager` 锁定的 pnpm 版本。

## 当前工作区状态（v0.8.4）

- v0.8.4：Codex Plugins 启停；供应商 Web Search；环境 doctor；Prompt 重命名；MCP registry 出 preview；Opus fast 定价与 Codex 会话 fast tier 成本匹配。
...
- 规划见 `task.md`；`task.md` / `bug.md` 为本地规划（gitignore）。
- 不要将 `release/` 调试文件或编译缓存纳入提交。

## 已验证结果

- `list_and_toggle_plugins_from_config_and_cache`、`normalize_usage_model_appends_fast_suffix`、`provider_can_disable_web_search_for_openai_upstream`：通过
- 前端 `tsc --noEmit`：通过

## 后续操作建议

1. 推送 `main` 与带注释的 `v0.8.4` 标签，由 GitHub Actions 云端构建发布。

...
**本版**：智能网关支持多套全局档案，每个 Agent 的 Auto 卡可选用不同配置（例如 Code「编程」、Codex「Astra」）。网关页可新建/复制/重命名档案；未绑定与自定义 Key 仍走默认档案。修复新建档案后下拉显示内部 id、切不过去的问题。修复路由模式被读空落到 gpt-6，以及已切到独立供应商后改网关规则仍写回 15828。其余同 1.5.3。

**升级注意**：资料库迁到 Schema 33（单向）。请先备份 `~/.claude-switcher`。1.4.xx 热修走 `release/1.4.x`，不回写 Schema 33。

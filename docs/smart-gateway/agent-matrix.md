...
| --- | --- | --- | --- | --- |
| Claude Code | 直连或现有协议代理 | 承接 opusplan + 规划/执行/辅助跨上游 | opus / sonnet / opusplan / haiku / subagent | 关闭分工后槽位无效 |
| Claude Desktop | 不变（Haiku 探测、429 原样） | 明确模型 + 辅助槽 | 无 opusplan 则不假装规划/执行 | 入口 token 鉴权 |
...
- gateway：Agent 只拿到本机入口 URL + 档案 token，拿不到全部上游密钥。
- Claude Code：仅当档案 `role_routing_enabled` 且规划或执行槽非空时写 `model=opusplan`。
